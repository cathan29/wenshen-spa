                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left">
                                ...
                                <tbody class="divide-y divide-[#F9F3E3]">
                                    ...
                                    {{-- 💡 ROW LOGIC: If disabled, dim the row --}}
                                    <tr class="group hover:bg-[#fffbf2] transition duration-200 {{ $service->is_active ? '' : 'opacity-60 bg-gray-50' }}">
                                        
                                        {{-- Name --}}
                                        <td class="p-6">
                                            <div class="font-serif font-bold text-lg text-[#6B4E31] flex items-center gap-2">
                                                {{ $service->service_name }}
                                                {{-- 💡 STATUS BADGE --}}
                                                @if(!$service->is_active)
                                                    <span class="px-2 py-0.5 rounded text-[10px] uppercase font-bold bg-gray-200 text-gray-500 tracking-wider">
                                                        Disabled
                                                    </span>
                                                @endif
                                            </div>
                                            <div class="text-[10px] {{ $service->is_active ? 'text-green-600' : 'text-gray-400' }} uppercase tracking-wider font-bold mt-1">
                                        ...
                                        <td class="p-6 text-right">
                                            <div class="flex justify-end items-center gap-3 opacity-60 group-hover:opacity-100 transition">
                                                ...
                                                <form action="{{ route('services.toggle', $service->id) }}" method="POST">
                                                    @csrf @method('PATCH')
                                                    
                                                    @if($service->is_active)
                                                        {{-- If Active -> Show Disable Button (Red) --}}
                                                        <button type="submit" class="text-red-400 hover:text-red-600 transition p-2 bg-white rounded-full shadow-sm hover:shadow-md border border-gray-100" title="Disable Service">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>
                                                        </button>
                                                    @else
                                                        {{-- If Inactive -> Show Enable Button (Green) --}}
                                                        <button type="submit" class="text-green-500 hover:text-green-700 transition p-2 bg-white rounded-full shadow-sm hover:shadow-md border border-gray-100" title="Enable Service">
